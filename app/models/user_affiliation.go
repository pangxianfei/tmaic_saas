...
func (uaff *UserAffiliation) InsertNode(user *User, fromCode ...string) error {
	...
	m.Transaction(func(TransactionHelper *helper.Helper) {
		...
		if current.FromCode == nil {
			...
			current.Root = &userid
			current.Parent = nil
			current.Level = ptr.Int64(1)
			current.Left = ptr.Int64(1)
			current.Right = ptr.Int64(2)
		...
		current.Root = parent.Root
		current.Parent = parent.UserID
		current.Level = ptr.Int64(*parent.Level + 1)

		current.Left = parent.Right
		current.Right = ptr.Int64(*parent.Right + 1)
		if err := TransactionHelper.Create(&current); err != nil {
			panic(err)
		}
		return
...
func (t *Tree) recursiveCombineTree(current Tree, level int64, nodes []UserAffiliation) []Tree {
